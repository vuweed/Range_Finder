...
org 000BH				;address at timer interrupt
jmp ISR_T0
start:
LCALL BUZZER_ISR
...
HERE1: JNB TF1,HERE1   // loops here until timer overflows (ie;48 count)
	 ...
	 ; LCALL COMPARE_FUNC
	 	////////////////////////////////////////////  
		  ;JMP COMPARE_FUNC
	  //CJNE R4, BLINK_LED_50ms
      LCALL DLOOP      // calls the display loop
...
CJNE R4, #0, NOT_EQUAL
	  ...
	  JC LESS
		  ...
		  NOT_EQUAL1:  
		  JC LESS1
		  ;CLR LED ; ranged > 20 cm
			//
		 
			  CJNE R4, #50, sub_NOT_EQUAL1
			  sub_NOT_EQUAL1:  
			  JC sub_LESS1
			  ;CLR LED ; ranged > 50 cm
			  ////
			  CJNE R4, #80, sub_sub_NOT_EQUAL1 
			  sub_sub_NOT_EQUAL1:  
			  JC sub_sub_LESS1
			  CLR LED ; range > 80 cm
			  JMP sub_sub_RET_COMPARE 
			  sub_sub_LESS1: 
			  CJNE R0,#50,continue
			  MOV R0,#0
			  CPL LED
...
delay_1s: ;1 chu ki may = 1us => 1s = 2*255*255*8
